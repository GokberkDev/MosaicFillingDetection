...
cv::Scalar ColorDetector::detectDominantColor(const cv::Mat& roi_bgr,
    ...
    int threshold = std::max(8, total_colored_pixels / 10);  // Daha yüksek eþik
    int max_count = threshold;
    cv::Scalar result_color = cv::Scalar(255, 255, 255);

    // Kýrmýzý için ÇOK DAHA YÜKSEK eþik
    if (red_count > max_count && red_count > total_colored_pixels / 4) {  // %25 eþik!
        max_count = red_count;
        result_color = cv::Scalar(0, 0, 255);
    }
    if (blue_count > max_count) { max_count = blue_count; result_color = cv::Scalar(255, 0, 0); }
    if (green_count > max_count) { max_count = green_count; result_color = cv::Scalar(0, 255, 0); }
    if (yellow_count > max_count) { max_count = yellow_count; result_color = cv::Scalar(0, 255, 255); }
    if (purple_count > max_count) { max_count = purple_count; result_color = cv::Scalar(255, 0, 255); }
    if (orange_count > max_count) result_color = cv::Scalar(0, 165, 255);
...
cv::Mat MosaicDetector::generateDigitalOutput(const cv::Mat& warped_frame) {
    // Sadece warped frame'i renklendir - contour yok
    cv::Mat hsv_warped;
    cv::cvtColor(warped_frame, hsv_warped, cv::COLOR_BGR2HSV);

    cv::Mat digital_output = warped_frame.clone();

    // Her pikseli renklendir
    for (int y = 0; y < digital_output.rows; y++) {
        for (int x = 0; x < digital_output.cols; x++) {
            cv::Vec3b hsv = hsv_warped.at<cv::Vec3b>(y, x);
            cv::Vec3b& pixel = digital_output.at<cv::Vec3b>(y, x);
            ...
            // KIRMIZI
            if (((h <= 7) || (h >= 173)) && r > 140 && r > g * 1.5 && r > b * 1.5) {
                pixel = cv::Vec3b(0, 0, 255);
            }
            // TURUNCU
            else if (h >= 8 && h <= 20 && r > 160 && g > 70 && g < 150) {
                pixel = cv::Vec3b(0, 165, 255);
            }
            // SARI
            else if (h >= 21 && h <= 35 && r > 160 && g > 160) {
                pixel = cv::Vec3b(0, 255, 255);
            }
            // YEÞÝL
            else if (h >= 40 && h <= 80 && g > 140 && g > r * 1.3) {
                pixel = cv::Vec3b(0, 255, 0);
            }
            // MAVÝ
            else if (h >= 95 && h <= 135 && b > 130 && b > r * 1.2) {
                pixel = cv::Vec3b(255, 0, 0);
            }
            // MOR
            else if (h >= 140 && h <= 165 && r > 120 && b > 120) {
